...
PIPER_MODEL_PATH = __import__("os").environ.get("PIPER_MODEL_PATH", "path/to/piper/model")
...
def _load_piper_model(model_path: str) -> Optional[Any]:
    """Load the Piper TTS model."""
    try:
        from piper import TextToSpeech  # type: ignore
...
async def _speak(text: str, tts: Optional[Any]) -> None:
    ...
    try:
        audio = tts.tts(text)
        # Play audio (this part needs to be implemented based on your environment)
        # For example, you can use sounddevice or pyaudio to play the audio
    except Exception as exc:
...
class VoiceInterface:
    ...
    def __init__(self) -> None:
        ...
        self._tts: Optional[Any] = _load_piper_model(PIPER_MODEL_PATH)
    ...
    async def start(self) -> None:
        ...
        try:
            await _speak(f"Jarvis Spracheingabe aktiv.", self._tts)
        except Exception:
            pass
    ...
    async def _listen_once(self) -> None:
        ...
        await _speak(response, self._tts)
